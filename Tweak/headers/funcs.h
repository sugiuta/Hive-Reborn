...
#define rads(angleDegrees) ((angleDegrees) * M_PI / 180.0) // 角度の計算 M_PIは円周率を表す。
#define val(p) ([NSValue valueWithCGPoint:p]) // NSValueをCGPointとして取得する。
#define domainStr @"com.sugiuta.hivereborn"
#define notifStr @"com.sugiuta.hivereborn-startUnlockAnimation"

// inline関数にすることによってソースコードが読みやすくなる。
inline CGFloat heightForWidth(CGFloat w) {
    return (w/2)*sqrt(3); // sqrt(3)は3の平方根を表す。
}
...
inline CGFloat sideLengthForWidth(CGFloat w) { // sLenは六角形の辺の横の長さ
...
NSArray* pointsForWidth(CGFloat w) {
    ...
    /**
     * NSArray* points = @[ val(p1), val(p2), val(p3), val(p4), val(p5), val(p6) ];
     * CGPointの構造体をNSArrayに入れている。p1 = 左、p4 = 右である。
     * val(p1)などはdefineで既にNSValueをCGPointとして取得されている。
     */
    return points;
}
